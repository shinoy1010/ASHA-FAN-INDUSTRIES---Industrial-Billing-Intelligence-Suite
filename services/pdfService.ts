...
async function fetchImageAsBase64(url: string): Promise<string> {
  ...
  try {
    ...
    if (!response.ok) {
      console.warn(`Logo fetch failed: HTTP ${response.status} for ${url}. PDF will generate without logo.`);
      return '';
    }
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onerror = (e) => {
        console.error("FileReader error while processing logo:", e);
        resolve(''); // Resolve empty to continue PDF generation
      };
      reader.readAsDataURL(blob);
    });
  } catch (error) {
    console.error("Network error while loading logo image:", error);
    return '';
  }
}
...
export const generateInvoicePDF = async (billNumber: string, allData: RowData[]) => {
  ...
  if (items.length === 0) {
    console.error(`No items found for bill number: ${billNumber}. Available bills:`, Array.from(new Set(allData.map(r => r['Bill Number']))));
    return;
  }
  ...
  try {
    ...
    // --- 2. COMPANY HEADER ---
    // Try multiple path patterns for the logo to be extra safe on Vercel
    const logoUrl = new URL('../components/logo.jpeg', window.location.origin).href;
